...
Since 0.5.0 the same automation also owns bare “stop” while a timer alarm is ringing. It first calls `echo_experience.dismiss_timer` with the originating device id; the integration resolves that id to an Echo profile server-side, including companion satellites listed in the profile's `ducking.additional_satellites`, via the entity registry. When the service returns `dismissed` it replies “Timer stopped.” and does not touch music; `unconfirmed`/`failed` reply that the alarm could not be confirmed stopped; anything else (nothing ringing, unknown device, service error) falls through to the music stop. Explicit music phrases (“stop music”, “stop Sonos”, …) skip the timer step entirely. Since 0.5.1 the automation has no timer or alarm phrases at all: “stop the timer”, “stop the alarm”, “dismiss the timer” and named dismissals (“stop the pasta timer”) go to the LLM's `echo_timer` tool, which can tell a ringing alarm from a running countdown and cancels or dismisses accordingly. An unconfirmed dismissal is reported as such rather than claimed. Devices outside the profiles keep the previous area-scoped Sonos behaviour and never call the timer service. The timer step uses `continue_on_error`, but Home Assistant still aborts a script when the service itself is missing, so while the Echo Experience integration is unloaded (reload/upgrade) bare “stop” returns an error instead of stopping music; explicit music phrases are unaffected.
